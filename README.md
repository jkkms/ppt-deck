...
| 폰트 크기 종류 남발 | 타입 스케일 9단. 값은 지어낸 게 아니라 **실제 덱 4종·137장에서 실측**했다 (린터 `SCALE`이 강제) |
| 글자가 쓸데없이 큼 | 제목 31pt · 본문 13pt. 표지만 46pt. 실측 최대가 72pt(1회)였다 |
| 장마다 제목 위치가 흔들림 | 수직 리듬 고정 — 눈썹 45 / 제목 68 / 본문 117. 훑을 때 눈이 안 흔들린다 |
...
hex 값은 팔레트당 **3개뿐**이고, 거기서 4단 톤 램프(`figure` → `ink2` → `muted` → `faint`)를 파생합니다. 작은 활자에서 위계를 세우려면 2단으로는 부족합니다. `muted`·`hairline`은 파생값이고, 혼합비를 명/암에서 다르게 가져가 대비를 보장합니다 (밝은 판 0.34 / 어두운 판 0.30 · ground ≥ 4.5:1, figure ≥ 1.7:1). 나머지 이름은 전부 그 3개의 의미론적 별칭입니다. 4번째 색이 필요해지면 색이 부족한 게 아니라 레이아웃이 틀린 것입니다.

| | 바탕 | 본문 | 강조 | 성격 |
| --- | --- | --- | --- | --- |
| `letterpress` | `#F4F1EA` | `#1B1B1F` | `#C8452D` | 편집디자인. 인쇄·배포물 |
| `forest` | `#FBFAF7` | `#161C18` | `#1F5D42` | 강의·교육자료 (실측 팔레트) |
